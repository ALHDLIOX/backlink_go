...
For normal batch execution, call `placement-history --product-id PRODUCT_ID --json` once after `doctor` and Campaign resolution. Build an in-memory index from the returned rows, use that index for all source URLs, and update it from every successful Placement readback. `--platform-domain` is a targeted refresh for interrupted, ambiguous, or externally changed state; it is not the default per-destination lookup pattern.
