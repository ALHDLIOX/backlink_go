...
For a historical `awaiting approval` row whose evidence explicitly proves no submission occurred, append an Event with action `correct unsubmitted status` and exact result `no submission confirmed`; explain the old misclassification in its evidence. Other result text, including contradictory or negated statements, is rejected. This confirmed correction establishes badge-queue history; leaving the queue still requires a later badge-verification resume Event. Existing HTTP(S) links block correction regardless of scheme casing or surrounding whitespace. Use `upsert-placement --correction-event-id EVENT_ID --input placement.json` to correct it to `waiting badge`. Supply the same evidence reference, current expected row version, `action_at: not submitted`, and the required badge follow-up. The CLI rejects other source/target states, missing or unrelated correction events, and existing public/backlink URLs. Do not use this to reopen real submissions. Dry run validates only the payload; live correction additionally checks the prior state and linked event.
...
A Placement cannot leave `waiting badge` by reusing its original defer/select Event. Append a newer linked Event with action `resume after badge verification`, matching evidence, and a semicolon-delimited result containing the exact positive tokens `badge verified` and `submitted`, `published`, or `submission attempted` as applicable. Live upsert and workbook audit both reject a missing, stale, mismatched, or incomplete resume Event.

A badge queue Event must use the badge-specific action `select badge launch` or `defer for badge`, or the exact result `waiting badge`; a generic `defer` action has no badge meaning. Resume result matching uses exact semicolon-delimited tokens, so negated phrases such as `not submitted` and `unpublished` never satisfy the positive outcome check.
