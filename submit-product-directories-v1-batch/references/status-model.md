...
Ordinary batches skip existing `waiting badge` Placements. Resume them only during the user's later badge-processing task: add badges in that authorized batch, verify each required badge is live, reopen the original route, recheck current pricing and conditions, refill the form, then submit under the applicable submission authorization. Reuse the same Placement/idempotency key and row-version checks; append Events as it progresses. If the badge is absent or invalid, retain `waiting badge`. Badge installation is not authorized by merely queueing the site.
