...
For authentication in every lane, follow [the shared login and registration policy](../submit-product-directories-v1-batch/references/authentication.md). The user has granted standing authorization for session reuse, the configured Google account's sign-in/sign-up, and matching Gmail verification; do not request renewed approval for these actions. This authorization does not grant publication permission.
